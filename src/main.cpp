...
extern const String firmwareVersion = "vShelly.4";
#ifndef UNIT_TEST
#include <EEPROM.h>
#include <TFT_eSPI.h>
#include <FS.h>
...
void setup()
{
  ...
  Config.title = "Shelly-Meter";
  Config.apid = "Shelly-Meter";
  Config.psk  = "12345678";
...
void loop()
{
  while(true){
    ...
    if (WiFi.status() != WL_CONNECTED) {
      ...
      if (hasShellyCredentials() && shellyDataUpdated) {
        shellyDataUpdated = false;
        renderTicker();
      }

      if (hasShellyCredentials() && (millis() - lastTimedTaskAt > timedTaskIntervalMs)) {
        readBatteryLevel();
